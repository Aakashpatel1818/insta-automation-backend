    ...
    while True:
        try:
            ...
            for post in due_posts:
                ...
                try:
                    # Get specific or active IG account
                    query = {"user_id": post["user_id"]}
                    if post.get("account_id"):
                        try:
                            query["_id"] = ObjectId(post["account_id"])
                        except Exception:
                            pass
                    else:
                        query["is_active"] = True
                    ig_account = await db["instagram_accounts"].find_one(query)
                    if not ig_account:
                        ig_account = await db["instagram_accounts"].find_one({"user_id": post["user_id"]})
